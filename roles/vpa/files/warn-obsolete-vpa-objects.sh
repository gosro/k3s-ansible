...
{
    k3s kubectl get clusterrole system:admission-controller &> /dev/null
    OBSOLETE_CLUSTERROLE_EXISTS=$?
    if [[ $ret -eq 0 ]]; then
        k3s kubectl get clusterrole system:admission-controller -o yaml 2>&1 | grep verticalpodautoscalers &> /dev/null
        OBSOLETE_CLUSTERROLE_EXISTS=$?
    fi

    k3s kubectl get clusterrolebinding system:admission-controller &> /dev/null
...
if [[ ${OBSOLETE_CLUSTERROLE_EXISTS} -eq 0 ]]; then
    ...
    echo
    echo "k3s kubectl get clusterrole system:admission-controller -o yaml"
    echo
fi
...
if [[ ${OBSOLETE_CLUSTERROLE_BINDING_EXISTS} -eq 0 ]]; then
    ...
    echo "You can inspect the object content by running"
    echo
    echo "k3s kubectl get clusterrolebinding system:admission-controller -o yaml"
    echo
fi
