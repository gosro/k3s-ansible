...
function print_help {
  ...
  echo "The 'print' action will print all resources that would be used by, e.g., 'k3s kubectl diff'."
...
for i in $COMPONENTS; do
  if [ $i == admission-controller-deployment ] ; then
    if [ ${ACTION} == create ] ; then
      (bash ${SCRIPT_ROOT}/gencerts.sh || true)
    elif [ ${ACTION} == delete ] ; then
      (bash ${SCRIPT_ROOT}/rmcerts.sh || true)
      (bash ${SCRIPT_ROOT}/delete-webhook.sh || true)
    fi
  fi
  if [[ ${ACTION} == print ]]; then
    ${SCRIPT_ROOT}/vpa-process-yaml.sh ${SCRIPT_ROOT}/manifests/$i.yaml
  else
    ${SCRIPT_ROOT}/vpa-process-yaml.sh ${SCRIPT_ROOT}/manifests/$i.yaml | k3s kubectl ${ACTION} -f - || true
  fi
done
